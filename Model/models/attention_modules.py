...
class MultiHeadDeformableAttention2D(nn.Module):
    ...
    def forward(self, query, reference_points):
        ...
        # 1. Create spatial value map and split into heads
        spatial_value = self.value_proj(query).view(N, H, W, self.num_heads, self.head_dim)
        spatial_value = spatial_value.permute(0, 3, 4, 1, 2) # (N, Heads, head_dim, H, W)
        spatial_value = spatial_value.reshape(N * self.num_heads, self.head_dim, H, W)

        # 2. Get sampling offsets and attention weights from the original query
        ...
        attention_weights = F.softmax(attention_weights, dim=-1) # Shape: (N, L, Heads, K)

        # 3. Calculate sampling grid
        ...
        # 4. Reshape grid and perform sampling
        ...
        # 5. Apply attention weights and combine
        ...
        # 6. Merge heads and apply final projection
...
# --- 3D Deformable Self-Attention Module (Unchanged) ---
...
class ImageToDepthCrossAttention(nn.Module):
    """
    Performs cross-attention where Image features are queries and
    Point Cloud features are keys/values. Now includes its own FFN.
    """
    ...
    def forward(self, img_feats, pc_feats):
        # Q = Image, K = Point Cloud, V = Point Cloud
...
class DepthToImageCrossAttention(nn.Module):
    """
    Performs cross-attention where Point Cloud features are queries and
    the (already attended) Image features are keys/values.
